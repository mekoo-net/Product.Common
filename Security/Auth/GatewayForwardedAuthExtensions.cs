...
/// <summary>
/// 下游服务（Bff 等）信任 Gateway 注入的身�?header，不再二次验 JWT�?
/// </summary>
...
{
    /// <summary>
    /// �?Gateway 转发�?<c>X-User-Id</c> / <c>X-Actor</c> �?header 构建 <see cref="ClaimsPrincipal"/>�?
    /// �?<c>[Authorize]</c> 使用；业务身份仍通过 <see cref="ICurrentAuth"/> 读取同一�?header�?
    /// </summary>
    public static IServiceCollection AddGatewayForwardedAuth(this IServiceCollection services)
    {
        ...
        {
            o.AddPolicy(ProductAuthPolicies.AccountUser, p =>
            {
